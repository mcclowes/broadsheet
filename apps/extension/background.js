...
async function saveAndNotify(options = {}) {
  const { selectionText = null, tab: explicitTab = null } = options;
  ...
  try {
    ...
    const { article, created, highlight } = await saveUrl(
      url,
      html,
      selectionText,
    );
    ...
    const titleLine = selectionText
      ...
      : alreadySaved
        ...
        : "Saved to Broadsheet";
    await notify(titleLine, title);
    return {
      ok: true,
      article,
      created: created !== false,
      highlight: highlight ?? "skipped",
    };
  } catch (err) {
...
const SELECTION_MENU_ID = "broadsheet-save-with-highlight";

function registerContextMenu() {
  if (!chrome.contextMenus) return;
  // Re-creating on every install/startup avoids "duplicate id" errors
